...
function updateAuthStatus(session) {
  if (session?.user?.email) {
    setStatus(`Signed in as ${session.user.email}. Changes will save to Supabase.`, "good");
    return;
  }

  setStatus("Not signed in. Log in as admin before uploading images or saving live changes.");
}
...
async function loadContent() {
  ...
  if (!client) {
    ...
    setStatus("Supabase is not configured yet. Saving will use this browser only.");
    renderForm();
    return;
  }

  const { data: sessionData } = await client.auth.getSession();
  updateAuthStatus(sessionData.session);
